...
interface CampaignDao {
    ...
    @Query("SELECT COUNT(*) FROM campaigns WHERE note = :note") suspend fun campaignsWithNote(note: String): Int
    ...
    @Query("SELECT EXISTS(SELECT 1 FROM launch_sessions WHERE campaignId = :id AND state IN ('LAUNCHED','NEEDS_CONFIRMATION','REVIEW_LATER'))") suspend fun hasPending(id: String): Boolean
    ...
    @Query("UPDATE launch_sessions SET state = 'NOT_APPLIED', confirmedEntryId = NULL, updatedAt = :now WHERE campaignId = :campaignId AND id != :keepId AND state IN ('LAUNCHED','NEEDS_CONFIRMATION','REVIEW_LATER')") suspend fun closeOtherPendingLaunches(campaignId: String, keepId: String, now: Long)
    ...
    @Query("SELECT * FROM drafts WHERE draftKind = 'CAMPAIGN_NEW' ORDER BY updatedAt DESC") suspend fun allNewDrafts(): List<DraftEntity>
